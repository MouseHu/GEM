...
class Logger:
	def __init__(self, name):
		make_dir(os.path.join(os.getenv("DDQ_LOGDIR","./"),'log'),clear=False)
		make_dir(os.path.join(os.getenv("DDQ_LOGDIR","./"),'log/text'),clear=False)
		if name is None: self.name = time.strftime('%Y-%m-%d-%H:%M:%S')
		else: self.name = name + time.strftime('-(%Y-%m-%d-%H:%M:%S)')

		log_file = os.path.join(os.getenv("DDQ_LOGDIR","./"),'log/text')+self.name+'.log'
	...
	def summary_init(self, graph, sess):
		make_dir(os.path.join(os.getenv("DDQ_LOGDIR","./"),'log/board'),clear=False)
		self.summary_writer = SummaryWriter(graph, sess, os.path.join(os.getenv("DDQ_LOGDIR","./"),'log/board/',self.name))
	...
	def save_npz(self, info, info_name, folder, subfolder=''):
		make_dir(os.path.join(os.getenv("DDQ_LOGDIR","./"),'log')+folder,clear=False)
		make_dir(os.path.join(os.getenv("DDQ_LOGDIR","./"),'log')+folder+'/'+self.name,clear=False)
		if subfolder!='':
			make_dir(os.path.join(os.getenv("DDQ_LOGDIR","./"),'log')+folder+'/'+self.name+'/'+subfolder,clear=False)
			save_path = os.path.join(os.getenv("DDQ_LOGDIR","./"),'log')+folder+'/'+self.name+'/'+subfolder
		else:
			save_path = os.path.join(os.getenv("DDQ_LOGDIR","./"),'log')+folder+'/'+self.name
