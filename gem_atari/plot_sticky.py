...
plot_colors_alg = [
	{
		...
		'Rainbow': np.array([0.56, 0.0, 1.0]),
		'CDDQN': np.array([0.56, 0.0, 1.0]),
		'C51': np.array([0.19,0.55,0.91]),
...
if __name__=='__main__':
	...
	parser.add_argument('--id', help='plot data id', type=str, default='paper/sticky/alien-rews')
	...
	for CI_level in [0.6,1.0]:
		for task in data.keys():
			...
			for id, alg in enumerate(data[task].keys()):
				...
				plt.plot(X[:length_alg], Y[:length_alg], color=alg_color, label=plot_legend[alg], zorder=len(list(plot_colors_alg.keys()))-id)
