...
class DQN:
    ...
    def create_model(self):
        ...
        def create_operators():
            self.target = tf.stop_gradient(
                self.rews_ph + (1.0 - self.done_ph) * (self.args.gamma ** self.args.nstep) * self.q_t)
            target = self.target
            if self.args.learn[-2:] == 'lb':
                self.lb_ratio = tf.less(target, self.q_lb_ph)
                if self.args.lb_type == 'hard':
            ...
            if self.args.learn[-2:] == 'lb':
                if self.args.lb_type == 'soft':
                    self.q_loss += tf.square(tf.maximum(self.q_lb_ph - self.q_acts, 0.0))
    ...
    def feed_dict(self, batch):
        ...
        # batch_obs, batch_obs_next, batch_actions, batch_rewards, batch_next_obs, batch_dones = \
        #     batch['obs0'], batch['obs1'], batch[
        #         'actions'], batch['rewards'], batch['obs1'], batch['terminals1']
        # feed_dict = {
        #     self.raw_obs_ph: batch_obs,
        #     self.raw_obs_next_ph: batch_obs_next,
