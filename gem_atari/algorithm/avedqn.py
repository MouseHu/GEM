...
class AveragedDQN:
	...
	def create_model(self):
		...
		def create_network():
			...
			with tf.variable_scope('target_1'):
				with tf.variable_scope('value'):
					if self.args.double:
						self.q_t_1 = tf.reduce_sum(value_net(self.obs_next_ph)*self.pi_next, axis=1, keepdims=True)
					else:
						self.q_t_1 = tf.reduce_max(value_net(self.obs_next_ph), axis=1, keepdims=True)

			with tf.variable_scope('target_2'):
				with tf.variable_scope('value'):
					if self.args.double:
						self.q_t_2 = tf.reduce_sum(value_net(self.obs_next_ph)*self.pi_next, axis=1, keepdims=True)
					else:
						self.q_t_2 = tf.reduce_max(value_net(self.obs_next_ph), axis=1, keepdims=True)

			self.q_t = (self.q_t_1+self.q_t_2)*0.5
		...
		def create_operators():
			...
			self.target_update_op_1 = tf.group([
				v_t.assign(v)
				for v, v_t in zip(get_vars('target_1'), get_vars('target_2'))
			])
			self.target_update_op_2 = tf.group([
				v_t.assign(v)
				for v, v_t in zip(get_vars('main'), get_vars('target_1'))
			])
			...
			self.target_init_op = tf.group([
				...
				v_t.assign(v)
				for v, v_t in zip(get_vars('main'), get_vars('target_2'))
			])
	...
	def target_update(self):
		self.sess.run(self.target_update_op_1)
		self.sess.run(self.target_update_op_2)
