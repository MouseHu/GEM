...
from stable_baselines.common.math_util import safe_mean, unscale_action, reward2return
    ...
    def evaluate(self, nb_trajs):
        ...
        if self.eval_env is not None:
            ...
            logger.logkv('eval_abs_qs_difference',
                         safe_mean(
                             [np.mean(np.abs([a - b for a, b in zip(x, y)])) for x, y in zip(eval_qs, eval_returns)]))
