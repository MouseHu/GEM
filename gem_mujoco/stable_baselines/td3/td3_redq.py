...
class TD3REDQ(OffPolicyRLModel):
    """
    ...
    def __init__(self, policy, env, gamma=0.99, learning_rate=3e-4, buffer_size=50000,
        ...
        super(TD3REDQ, self).__init__(policy=policy, env=env, replay_buffer=None, verbose=verbose,
        ...
        with SetVerbosity(self.verbose):
            ...
            with self.graph.as_default():
                ...
                with tf.variable_scope("model", reuse=False):
                    ...
                    qf3, qf4 = self.policy_tf.make_critics(self.processed_obs_ph, self.actions_ph, scope="values_fn_2")
                ...
                with tf.variable_scope("target", reuse=False):
                    ...
                    qf1_target, qf2_target = self.target_policy_tf.make_critics(self.processed_next_obs_ph,
                                                                                noisy_target_action)
                    qf3_target, qf4_target = self.target_policy_tf.make_critics(self.processed_next_obs_ph,
                                                                                noisy_target_action,
                                                                                scope="values_fn_2")
                ...
                with tf.variable_scope("loss", reuse=False):
                    ...
                    # min_qf_target = tf.minimum(qf1_target, qf2_target,qf3_target, qf4_target)
                    stack_targets = tf.stack([qf1_target, qf2_target, qf3_target, qf4_target])
                    random_target = self._random_choice(stack_targets, 2)
                    min_qf_target = tf.reduce_min(random_target, axis=0)
                    ...
                    qf3_loss = tf.reduce_mean((q_backup - qf3) ** 2)
                    qf4_loss = tf.reduce_mean((q_backup - qf4) ** 2)
                    ...
                    qvalues_params = tf_util.get_trainable_vars('model/values_fn/')+\
                                     tf_util.get_trainable_vars('model/values_fn_2/')
